...
Converter.prototype.convert = function(tableData) {
    var wpd = {data:{},duplicates:[],folders:[]};
    ...
    _.each(tableData, function(data, url) {
        var ec = new EntityConverter(data, url);

        var token = ec.getTokenName(),
            p     = ec.getParentName(),
            row   = {};
        ...
        if (!_.contains(wpd['folders'], p)) {
          wpd['folders'].push(p);
        }

        row.breadcrumb = ec.getBreadcrumb();
        row.jsonselect = ':root > .data > .' + p + ' > .' + token;
        row.contents   = ec.getContents();
        row.links      = ec.getLinks();

        if (!_.has(wpd['data'][p], token)) {
          wpd['data'][p][token] = row;
        } else {
          // Ignoring two pages that are exactly the same, not flagging as
          //   duplicate.
          // https://developer.mozilla.org/en-US/docs/Web/API/element
          // https://developer.mozilla.org/en-US/docs/Web/API/Element/
          if ( p === 'webapi' && token === 'element' ) {
            return; // Exiting there
          }

          wpd['duplicates'].push({token: token, folder: p, data: row});
        }
      }
    );
