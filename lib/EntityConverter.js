...
/**
 * Hacky* getParentName (will do for now)
 *
 * - Read this.breadcrumb that is everything from this.origin after '/docs', toLowerCase, as an array
 * - Take the value of the -2 index of the breadcrumb property (original sorting)
 * - If "original sorting" has (replace with):
 *    - api -> webapi
 *    - (nothing) -> webapi
 *    - javascript_typed_arrays -> javascript
 *    - global_objects -> javascript
 *    - element -> replace by this.getBreadcrumb() that removes both 'element','web' from its members
 *
 * Use-case examples: http://jsbin.com/cuzan/9/edit?js,console
 **/
function makeBreadcrumb ( origin ) {
  // remove this part: https://developer.mozilla.org/en-US/docs/
  // and make it an ordered array
  var w = (origin.substr(origin.indexOf('/docs') + 6, origin.length)).toLowerCase(),
      breadcrumb      = [];

  w = w.replace('server-sent_events', 'webapi/server-sent-events');
  w = w.replace('guide/','');
  w = w.replace('reference/','');
  w = w.replace(/\/api\//, '/webapi/');
  w = w.replace('javascript_typed_arrays/', 'webapi/');
  w = w.replace('global_objects/', '');
  w = w.replace('core_javascript_1.5_reference/', 'javascript/');
  w = w.replace('web/','');
  w = w.replace(/\/::?/, "/pseudo-");
  w = w.replace(/\/dom/, "/dom-");
...
function EntityConverter(data, origin){
  this.data       = data;
  this.origin     = origin;
  this.breadcrumb = makeBreadcrumb(origin);
}
...
EntityConverter.prototype = {
  ...
  getTokenName: function getTokenName ( ) {
    ...
    var out = _.clone(this.breadcrumb).pop();

    out = _S(out)
            .replaceAll('.','-') /* try .dasherize() next time */
            .slugify();
  ...
  getLinks: function getLinks ( ) {
    ...
    out.push({title:"Original article on MDN in " + this.breadcrumb.join(' '), url: origin});
...
EntityConverter.prototype.getContents = function getContents ( ) {
    return convertData(this.data);
};

EntityConverter.prototype.getBreadcrumb = function getBreadcrumb ( ) {
