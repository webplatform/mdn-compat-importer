      ...
      _.forEach(b, function(c, ck) {
        ...
        _.forEach(c, function(d, dk){
          ...
          var valueSpaceIndex        = value.indexOf(' '),
              ...
              valuePosParenthesis    = value.indexOf('('),
              valueHasParenthesis    = (valuePosParenthesis >= 1)?true:false,
              valueParenthesisText   = null;

          if(valueHasParenthesis == true) {
            valueParenthesisText = value.between('(',')');
          }

          // Rewrite RdXX:
          //   - description: Keep what is inside parenthesis
